...
k<-length(beta)-1
replicas_delta = vector(mode='list', 3)
contador_falhas = 0

#Simulação utilizando GOSOLNP
for(n in n_){
  ...
  while(i<=R){
    ...
    inf<-c(rep(0.03,5),rep(min(mqo),length(mqo)))
    sup<-c(rep(10,5),rep(max(abs(mqo)),length(mqo)))
    tmp1<-try(gosolnp(fun=LQV.Reg.EGEDD,n.restarts=nsp,LB=inf,UB=sup,
                         n.sim=mint,dados=y,X=X,tau=tau,q=0.95,
                         control=list(trace=F)))
    if(class(tmp1) == "try-error"){
      contador_falhas=contador_falhas+1
    }else{
      theta = tmp1$pars
      tmp<-try(optim(theta,fn=LQV.Reg.EGEDD,lower=inf,upper=sup,
  ...
  replicas_delta[[match(n,n_)]] =  estimativa_parametros
}

# Avaliando a simulação
resultados_delta = data.frame(matrix(ncol = 8, nrow  = 0))
colnames(resultados_delta) = c('mean', 'sd' ,'parametros', 'Vies', 'EQM',
                         'Assimetria', 'Curtose', 'N')
j = 1
for(df in replicas_delta){
 df_ = df |> 
    colMeans() %>%
    cbind(apply(df, 2, sd)) %>%
    as.data.frame() |> 
 ...
 resultados_delta = resultados_delta %>%
   add_row(df_)
 j = j +1
}
